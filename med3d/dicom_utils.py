...
class Reader:
    ...
    def read_series(self, fn: str):
        ...
        dicom_names = series_reader.GetGDCMSeriesFileNames(str(fn))
        series_reader.SetFileNames(dicom_names)
        im = series_reader.Execute()
...
def equal_number_of_slices(fn_image, fn_mask):
    ...
    series_reader = sitk.ImageSeriesReader()
    dicom_names = series_reader.GetGDCMSeriesFileNames(str(fn_image))
...
def crop(image, margin, interpolator=sitk.sitkLinear):
    ...
    if all([isinstance(m, float) for m in _flatten(margin)]):
        assert all(
            [m >= 0 and m < 0.5 for m in _flatten(margin)]
