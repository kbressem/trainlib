...
def resample_to_ras_and_spacing(image: sitk.Image, 
                                spacing: List[int] = None, 
    ...
    resampled_image = sitk.Resample(image, output_size, euler3d, method, 
                                    output_origin, output_spacing, output_direction)
    for k in image.GetMetaDataKeys():
        resampled_image.SetMetaData(k, image.GetMetaData(k))
    return resampled_image 

def indices_for_patches(patch_edge_length: int, 
...
def store_old_affine(patch: sitk.Image, 
...
def image_to_patches(image: sitk.Image, 
                     patch_size: List[int]) -> List[sitk.Image]:
...
def string_tuple_to_numeric(string_tuple: str) -> Tuple[Union[int, float]]:
    ...
    return tuple(map(_float_or_int, re.sub("\(|\)| ", "", string_tuple).split(",")))

def patches_to_image(patches: List[sitk.Image], 
                     meta_dict: Optional[dict] = None) -> sitk.Image:
